...
from ..schemas.user_schema import UpdateUserProfileSchema, UpdateOrgProfileSchema, UpdateAdminProfileSchema, ChangePasswordSchema
...
def get_own_profile_service(account_id):
    ...
    if account.role == Role.USER:
        ...
        response.update({
            ...
            "gender": user_details.gender.value if user_details.gender else None,
            "date_of_birth": user_details.date_of_birth.strftime('%Y-%m-%d') if user_details.date_of_birth else None,
            ...
            "identity_verification_status": user_details.identity_verification_status.value if user_details.identity_verification_status else None
        })
    elif account.role == Role.ORGANIZATION:
        org_details = OrganizationDetails.query.filter_by(account_id=account_id).first()
        if not org_details:
        ...
        admin_details = AdminDetails.query.filter_by(account_id=account_id).first()
...
def update_profile_service(account_id, data):
    try:
        ...
        if new_username and new_username != account.username:
            if Account.query.filter(Account.username == new_username, Account.id != account_id).first():
        ...
        if account.role == Role.USER:
            ...
            if not user_details:
                return {"msg": "User details not found"}, 404
            for field in ['name', 'last_name', 'phone_number', 'gender', 'city', 'village', 'bio', 'experience', 'date_of_birth', 'profile_picture', 'identity_picture']:
                if field in validated_data and getattr(user_details, field) != validated_data[field]:
            ...
            for field in ['name', 'description', 'phone', 'logo', 'address']:
                if field in validated_data and getattr(org_details, field) != validated_data[field]:
            ...
            for field in ['name', 'role_level']:
                if field in validated_data and getattr(admin_details, field) != validated_data[field]:
...
def change_password_service(account_id, data):
    try:
        ...
        if not checkpw(old_password.encode('utf-8'), account.password.encode('utf-8')):
